// src/components/GasBillDialog.jsx
...
const GAS_PROVIDERS = {
  "Andhra Pradesh": ["HP Gas", "Bharat Gas", "Indane"],
  "Telangana": ["HP Gas", "Indane"],
  "Karnataka": ["HP Gas", "Bharat Gas", "Indane"],
  "Kerala": ["Bharat Gas", "Indane"],
  "Maharashtra": ["Indane", "HP Gas", "Bharat Gas"],
  "Tamil Nadu": ["Indane", "HP Gas", "Bharat Gas"],
  "Delhi": ["Indane", "Bharat Gas", "HP Gas"],
  "Uttar Pradesh": ["Bharat Gas", "Indane", "HP Gas"],
  "Gujarat": ["Sabarmati Gas", "Adani Gas", "HP Gas"],
  "Punjab": ["HP Gas", "Indane"],
  "Rajasthan": ["HP Gas", "Indane"],
  "Madhya Pradesh": ["Bharat Gas", "HP Gas"],
  "Bihar": ["Bharat Gas", "Indane"],
  "West Bengal": ["Indane", "Bharat Gas"],
};

export default function GasBillDialog({ open, onClose }) {
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md" PaperProps={{ sx: { borderRadius: 2, minHeight: 600 } }}>
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <LocalGasStationIcon sx={{ color: "#ca8a04" }} />
          <Typography variant="h6" fontWeight={700}>Gas Bill Payment</Typography>
        </Box>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <Chip
        ...
        <Stack spacing={3}>
          <FormControl fullWidth>
            ...
            <Select
              ...
              value={state}
              label="State"
              onChange={(e) => { setState(e.target.value); setProvider(""); setBillAmount(null); }}
            >
              {Object.keys(GAS_PROVIDERS).map((s) => (
          ...
          <FormControl fullWidth>
            ...
            <Select
              ...
              value={provider}
              label="Provider"
              onChange={(e) => { setProvider(e.target.value); setBillAmount(null); }}
              disabled={!state}
            >
              {state && GAS_PROVIDERS[state].map((p) => (
          ...
          <Button
            fullWidth
            variant="contained"
            sx={{ py: 1.8, fontWeight: 600, textTransform: "none", fontSize: "1.1rem", bgcolor: "#ca8a04", '&:hover': { bgcolor: '#a16207' } }}
            onClick={fetchBill}
