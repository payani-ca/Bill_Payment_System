// src/components/FastTagDialog.jsx
...
const FASTTAG_PROVIDERS = [
  "ICICI FastTag", "Paytm FastTag", "HDFC FastTag", "SBI FastTag",
  "Axis Bank FastTag", "Kotak FastTag", "IDFC First FastTag", "Airtel Payments Bank FastTag"
];

export default function FastTagDialog({ open, onClose }) {
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md" PaperProps={{ sx: { borderRadius: 2, minHeight: 500 } }}>
      <DialogTitle
        sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", bgcolor: "#fefce8", borderBottom: "1px solid #eaeaea" }}
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <CommuteIcon sx={{ color: "#ca8a04" }} />
          <Typography variant="h6" fontWeight={700}>FastTag Recharge</Typography>
        </Box>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <Chip
      ...
      <DialogContent sx={{ pt: 3, pb: 3, px: 4 }}>
        <Stack spacing={3}>
          <FormControl fullWidth>
            ...
            >
              {FASTTAG_PROVIDERS.map(p => <MenuItem key={p} value={p}>{p}</MenuItem>)}
            </Select>
          </FormControl>
