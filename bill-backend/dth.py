...
dth_bp = Blueprint("dth", __name__, url_prefix="/dth")
...
@dth_bp.route("/bill", methods=["POST"])
@jwt_required()
def dth_bill():
...
@dth_bp.route("/service_no", methods=["POST"])
@jwt_required()
def get_service_no():
    data = request.get_json(force=True)
    user_id = get_jwt_identity()
