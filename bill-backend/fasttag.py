...
# -------------------- SERVICE NUMBER --------------------
@fasttag_bp.route("/service_no", methods=["POST"])
@jwt_required()
def get_service_no():
    ...
    service_no = f"F-{sum_ascii('fasttag')}-{sum_ascii(provider)}-{sum_ascii(user_id)}-{sum_ascii(today)}"

    return jsonify({"service_no": service_no, "userID": user_id, "provider": provider}), 200

# -------------------- BILL GENERATION --------------------
@fasttag_bp.route("/bill", methods=["POST"])
@jwt_required()
def generate_bill():
    data = request.get_json(force=True)
    user_id = get_jwt_identity()
   ...
   # ServiceNo = data.get("ServiceNo")
    ...
    if provider not in UTILITIES["fasttag"]:
        return jsonify({"msg": "Invalid provider"}), 400

    amount = sum_ascii("fasttag") + sum_ascii(provider) + sum_ascii(user_id) + sum_ascii(today)
    if amount > 1000:
        amount = amount / 10
...
# -------------------- PAYMENT --------------------
@fasttag_bp.route("/pay", methods=["POST"])
@jwt_required()
def pay_bill():
    data = request.get_json(force=True)
    user_id = get_jwt_identity()
