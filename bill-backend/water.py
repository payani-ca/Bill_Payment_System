...
# -------------------- BILL GENERATION --------------------
@water_bp.route("/bill", methods=["POST"])
@jwt_required()
def generate_bill():
    ...
    service_no = f"W-{sum_ascii('water')}-{sum_ascii(provider)}-{sum_ascii(user_id)}-{sum_ascii(today)}"

    if not city or not provider:
        return jsonify({"msg": "Missing city or provider"}), 400

    # expected_service_no = f"W-{sum_ascii('water')}-{sum_ascii(provider)}-{sum_ascii(user_id)}-{sum_ascii(today)}"
    #
    # # Validate ServiceNo
    ...
    valid_cities = UTILITIES["water"]
    if city not in valid_cities or provider not in valid_cities[city]:
        return jsonify({"msg": "Invalid city or provider"}), 400

    # Generate mock bill amount
    amount = sum_ascii("water") + sum_ascii(provider) + sum_ascii(user_id) + sum_ascii(today)
    if amount > 1000:
        amount = amount / 10
...
# -------------------- PAYMENT --------------------
@water_bp.route("/pay", methods=["POST"])
@jwt_required()
def pay_bill():
    data = request.get_json(force=True)
    user_id = get_jwt_identity()

    required = ["city", "provider", "mobile", "bill_amount", "mpin", "service_no"]
    ...
    vendor = f"Water : {data['city']} : {data['provider']}"
    tx = {
        "TxID": str(uuid.uuid4()),
        "UserID": user_id,
        "Utility": "Water",
...
@water_bp.route("/service_no", methods=["POST"])
@jwt_required()
def get_service_no():
    data = request.get_json(force=True)
    user_id = get_jwt_identity()
